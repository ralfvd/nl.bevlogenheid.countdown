...
module.exports = {
    init: function(homeyCD) {
        ...
        homeyCD.settings.on('set',function (action) {
                  //console.log(action)
                  if (action == 'deleteall') {
                      //deleteAllInsights();
                      util.cdLog('delete all started')
                      deleteAllTokens();
                      homeyCD.settings.set('variables', []);
                  }

                  if (action == 'changedvariables' ) {
                    util.cdLog('set variable management')
                    var changeObject = HomeyCD.get('changedvariables');
...
function processValueChanged(variables, oldVariable, newVariable) {
    ...
    homeyCD.settings.set('variables', variables);
    //console.log(newVariable)
...
function createToken(name, value, type, pause) {
    if (type !== 'trigger' ) {
        ...
        token.register()
            ...
            .catch(err => {
                console.log("---Error createToken-START-")
                console.log(token)
                console.log(name);
                console.log(type);
                console.log(err);
                console.log("---Error createToken-END-")
            });
    }
}
...
function getVariables() {
    var varCollection = homeyCD.settings.get('variables');

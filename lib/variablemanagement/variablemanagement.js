...
function processValueChanged(variables, oldVariable, newVariable) {
    ...
    if (newVariable && oldVariable && newVariable.value !== oldVariable.value && newVariable.value !== -1) {
        getTrigger('countdown_reaches_number').trigger(null, { "variable": newVariable.name, "value": newVariable.value });
    }
    //countdown_started
    ...
    if (type !== 'trigger') {
        try {
            ...
            try {
                ...
                console.error(error);
                console.log("token.setValue("+name+"value) = " + (value? JSON.stringify(value) : 'UNDEFINED'));
            }
        } catch (error2) {
            console.error(error2);
            console.log("token.createToken("+name+", value) = " + (value? JSON.stringify(value) : 'UNDEFINED'));
        }
    }
}

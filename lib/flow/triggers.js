...
module.exports = {
  ...
  let countdown_to_zero = homey.flow.getTriggerCard('countdown_to_zero');
    ...
    //countdown_reaches_number
    let countdown_reaches_number = homey.flow.getTriggerCard('countdown_reaches_number');
    countdown_reaches_number
    .registerRunListener((args, state) => {
      if (args.variable) {
        //  console.log ('state:' + state.variable);
          //var variable = variableManager.getvariable(args.variable.name);
        //  console.log ('args:' + args.variable.name)
          if (args.value === state.value && state.variable == args.variable.name) {
              homey.log('flow: countdown_reaches_number');
              return true;
          }
        }
        //If true, this flow should run
        return Promise.resolve( args.variable.name === state.variable && state.value == args.value ).catch(() => {
           console.error('Do that');
          })
        })
      .getArgument('variable')

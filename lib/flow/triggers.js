...
module.exports = {
  ...
  CreateFlowCardTriggers: function (_homey, variableManager) {
    ...
    let retrieveCountdowns = (query, args) => {
      let l = variableManager.getVariables().filter(util.contains(query));
      if(query && query.length && !l.find(x=>x.name.toLowerCase()===(''||query).toLowerCase())) {
        l.unshift({name:query, type:'number', description:'Create new countdown'});
      } //else lastQuery = null;

      if(lastQuery && lastQuery.length && !l.find(x=>x.name.toLowerCase()===(''||lastQuery).toLowerCase())) {
        l.unshift({name:lastQuery, type:'number', description:'Just added'});
      }
      lastQuery = query;
